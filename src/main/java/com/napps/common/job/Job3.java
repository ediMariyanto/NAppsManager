...
public class Job3 extends QuartzJobBean {
	private static org.apache.log4j.Logger log = Logger.getLogger(Job3.class);
	...
	protected void executeInternal(JobExecutionContext job)
		...
		try {
			...
			String jobs = document.getElementsByTagName("pathJob3").item(0).getTextContent();

			if (jobs.equals("")) {
				log.warn("'job3' is empty !");
			} else {
				Runtime r = Runtime.getRuntime();
				r.exec(jobs);

				log.info("'JOB 3' Running at : "
						+ job.getFireTime() + " \t"
						+ "- and next Running at : "
						+ job.getNextFireTime());
			}
			...
			ex.printStackTrace();
			log.error("Configuration 'JOB3' not valid " + ex);
		}
	}
}
